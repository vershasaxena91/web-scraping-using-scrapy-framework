...
class Converter:
    def __init__(self, filename):  # , model):
        ...
        self.prod_moves = pd.DataFrame(
            columns=[
                ...
                "Last Known Lowest Selling Price",
                "Last Known Highest Selling Price",
                "BSR",
                "Last Known BSR",
            ]
        )
    ...
    def convert_doc(self, i):
        ...
        def convert_string_to_bsr(sbsr):
            ...
            """
            e1 = re.search("#[0-9,]+ in Women's Kurtas & Kurtis", sbsr) or re.search("#[0-9,]+ in Women's Salwar Suits", sbsr)
        ...
        prod_price_move = pd.DataFrame(
            columns=["ASIN", "Date", "Lowest Selling Price", "Highest Selling Price", "Last Known Lowest Selling Price", "Last Known Highest Selling Price"]
        )
        ...
        try:
            for p in self.data[i]["product_sale_price"]:
                # print("*******")
                # print(type(p["value"]))
                # p_low = float(p["value"].split("-")[0]) if "-" in p["value"] else float(p["value"])
                # print(p_low)
                try:
                    # p["value"] = float(p["value"].replace(",", "")) 
                    if p["value"] != "":
                        p_low = float(p["value"].split("-")[0]) if "-" in p["value"] else float(p["value"])
                        p_high = float(p["value"].split("-")[1]) if "-" in p["value"] else float(p["value"])
                    else:
                        p_low = np.nan
                        p_high = np.nan
                ...
                price_list.append(
                    {
                        ...
                        "Lowest Selling Price": p_low,
                        "Highest Selling Price": p_high,
                        # "Lowest Selling Price": float(p["value"].split("-")[0]) if "-" in p["value"] else float(p["value"]),
                        # "Highest Selling Price": float(p["value"].split("-")[1]) if "-" in p["value"] else float(p["value"]),
                ...
                if p["value"] is not None:
                    if (
                        ...
                        last_price_high = price_high
                        last_day = day
                    day = p["time"]
                    price_low = p_low
                    price_high = p_high
                    # price_low = float(p["value"].split("-")[0]) if len(p["value"].split("-"))==2 else float(p["value"])
                    # price_high = float(p["value"].split("-")[0]) if len(p["value"].split("-"))==2 else float(p["value"])
        except:
            pass
        prod_price_move = prod_price_move.append(price_list, ignore_index=True)
        prod_price_move = pd.DataFrame(
            prod_price_move.groupby(["ASIN", "Date"])["Lowest Selling Price", "Highest Selling Price"].agg(
        ...
        # for j in range(len(prod_move["Lowest Selling Price"])):
        ...
        try:
            prod_move["Lowest Selling Price"] = prod_move["Lowest Selling Price"].round()
            prod_move["Highest Selling Price"] = prod_move["Highest Selling Price"].round()
        except:
            pass
        ...
        prod_move["Last Known Lowest Selling Price"] = last_price_low
        prod_move["Last Known Highest Selling Price"] = last_price_high
        ...
        try:
            ...
            # )
            if "-" in self.data[i]["product_original_price"][-1]["value"]:
                self.data[i]["product_original_price"] = (self.data[i]["product_original_price"][-1]["value"]).split("-")
            else:
                self.data[i]["product_original_price"] = (self.data[i]["product_original_price"][-1]["value"])
        except:
            ...
            [
                {
                    ...
                    "Lowest Original Price": self.data[i]["product_original_price"][0] if len(self.data[i]["product_original_price"])==2 else self.data[i]["product_original_price"],
                    "Highest Original Price": self.data[i]["product_original_price"][1] if len(self.data[i]["product_original_price"])==2 else self.data[i]["product_original_price"],
                    "Latest Lowest Selling Price": price_low,
                    "Latest Hightest Selling Price": price_high,
                    "Last Day Lowest Selling Price": last_price_low,
                    "Last Day Highest Selling Price": last_price_high,
                    "Latest BSR": bsr,
...
if __name__ == "__main__":
    ...
    cvt = Converter(
        "../../data/InputData/biba/amazon_marketplace_scraping/product_data.json")
        # "../../sentiment_analysis/XGBoost_Model_Biba.json")
    cvt.convert()
    cvt.save(
        "../../data/OutputData/biba/amazon_marketplace_scraping/amazon_prod_info.csv",
        "../../data/OutputData/biba/amazon_marketplace_scraping/amazon_prod_move.csv",
    )
