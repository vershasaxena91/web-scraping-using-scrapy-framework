...
class Converter:
    ...
    def convert_doc(self, i):
        ...
        def convert_string_to_bsr(sbsr):
            ...
            # e1 = (
            #     re.search("#[0-9,]+ in Emergency Lights", sbsr)
            #     or re.search("#[0-9,]+ in Tube Lights & Battens", sbsr)
            #     or re.search("#[0-9,]+ in LED Bulbs", sbsr)
            #     or re.search("#[0-9,]+ in Desk Lights", sbsr)
            #     or re.search("#[0-9,]+ in Table Lamps", sbsr)
            #     or re.search("#[0-9,]+ in Smart Light Bulbs", sbsr)
            #     or re.search("#[0-9,]+ in Lighting Fixtures", sbsr)
            #     or re.search("#[0-9,]+ in LED Strip Lights", sbsr)
            # )
            ...
            """
            # Lighting
            e1 = re.search("#[0-9,]+ in Emergency Lights", sbsr) or re.search("#[0-9,]+ in Tube Lights & Battens", sbsr) or re.search("#[0-9,]+ in LED Bulbs", sbsr) or re.search("#[0-9,]+ in Desk Lights", sbsr) or re.search("#[0-9,]+ in Table Lamps", sbsr) or re.search("#[0-9,]+ in Smart Light Bulbs", sbsr) or re.search("#[0-9,]+ in Lighting Fixtures", sbsr) or re.search("#[0-9,]+ in LED Strip Lights", sbsr)
            """
        ...
        if not isinstance(self.data[i]["product_brand"], str) or self.data[i][
            ...
            if "Blue Star" in self.data[i]["product_name"]:
                ...
                self.data[i]["product_brand"] = "L'Oreal Paris"
            elif "Dove" in self.data[i]["product_name"]:
                self.data[i]["product_brand"] = "Dove"
            elif "L'Oréal Professionnel" in self.data[i]["product_name"]:
                self.data[i]["product_brand"] = "L'Oréal Professionnel"
...
if __name__ == "__main__":
    ...
    cvt = Converter(
        "../../data/InputData/safed_detergent_powder/amazon_marketplace_scraping/product_data.json",
        "../../sentiment_analysis/XGBoost_Model_Detergent_Powder.json",
    )
    cvt.convert()
    cvt.save(
        "../../data/OutputData/safed_detergent_powder/amazon_marketplace_scraping_new/amazon_prod_info.csv",
        "../../data/OutputData/safed_detergent_powder/amazon_marketplace_scraping_new/amazon_prod_move.csv",
    )
