...
collec = client["amazon_marketplace_scraping_sugar_free"]["product_list"]

prods_df = pd.read_csv("Amazon_Sugar_Free.csv")
...
for product in products:
    ...
    client_added = collec.find_one(
        {"product_asin": product["product_asin"], "clients": "Sugar_Free"}
    )
    if not client_added:
        collec.find_one_and_update(
