...
collec = client["amazon_marketplace_scraping_colgate"]["product_list"]

prods_df = pd.read_csv("Amazon_Colgate.csv")
...
for product in products:
    ...
    client_added = collec.find_one(
        {"product_asin": product["product_asin"], "clients": "Colgate"}
    )
    if not client_added:
        collec.find_one_and_update(
