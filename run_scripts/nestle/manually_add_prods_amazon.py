...
collec = client["amazon_marketplace_scraping_nestle"]["product_list"]

prods_df = pd.read_csv("Nestle_Flipkart_Amazon_Match.csv")
...
for product in products:
    ...
    client_added = collec.find_one(
        {"product_asin": product["product_asin"], "clients": "Nestle"}
    )
    if not client_added:
        collec.find_one_and_update(
