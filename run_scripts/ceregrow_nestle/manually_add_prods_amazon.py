...
collec = client["amazon_marketplace_scraping_ceregrow"]["product_list"]

prods_df = pd.read_csv("Ceregrow_Flipkart_Amazon_Match.csv")
...
for product in products:
    ...
    client_added = collec.find_one(
        {"product_asin": product["product_asin"], "clients": "Ceregrow"}
    )
    if not client_added:
        collec.find_one_and_update(
