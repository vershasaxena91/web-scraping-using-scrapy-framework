...
collec = client["amazon_marketplace_scraping_hafele"]["product_list"]

prods_df = pd.read_csv("Hafele_Flipkart_Amazon_Match.csv")
...
for product in products:
    ...
    client_added = collec.find_one(
        {"product_asin": product["product_asin"], "clients": "Hafele"}
    )
    if not client_added:
        collec.find_one_and_update(
