...
collec = client["amazon_marketplace_scraping_eno"]["product_list"]

prods_df = pd.read_csv("Eno_Flipkart_Amazon_Match.csv")
...
for product in products:
    ...
    client_added = collec.find_one(
        {"product_asin": product["product_asin"], "clients": "Eno"}
    )
    if not client_added:
        collec.find_one_and_update(
